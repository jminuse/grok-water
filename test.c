	...
	for(step=0; step<10000; step++) {
		...
		for(i=0; i<n_molecules; i++) { //update velocities and accelerations
			...
			for(j=0; j<n_molecules; j++) {
				...
				double r = sqrt(len_squared_(rot_dr));
				double theta = atan2(rot_dr.y,rot_dr.x);
				double phi = acos(rot_dr.z/r);
				
				double rx = atan2(dA.m[2][1], dA.m[2][2]);
				double ry = atan2(-dA.m[2][0], sqrt(dA.m[2][1]*dA.m[2][1] + dA.m[2][2]*dA.m[2][2]));
				double rz = atan2(dA.m[1][0], dA.m[0][0]);
				
				potential_element e = interpolate_water_potential(r, theta, phi, rx, ry, rz);
				vector torque = (vector){e.tx, e.ty, e.tz};
				
				vector acceleration = multiply_(dr, -e.acceleration/water_mass); //restore to global coordinates
